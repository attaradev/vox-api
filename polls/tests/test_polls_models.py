...
def test_poll_status_transition_requires_choices():
    ...
    question = Question.objects.create(poll=poll, text="Question 1")

    with pytest.raises(ValidationError):
        poll.transition_status(Poll.Status.ACTIVE)

    Choice.objects.create(question=question, text="Pizza", order=0)
    poll.transition_status(Poll.Status.ACTIVE)
...
def test_poll_cannot_reopen_after_closed():
    ...
    question = Question.objects.create(poll=poll, text="Question 1")
    Choice.objects.create(question=question, text="Option", order=0)
    poll.transition_status(Poll.Status.ACTIVE)
...
def test_vote_validation_enforces_choice_relationship():
    ...
    question_one = Question.objects.create(poll=poll_one, text="Q1")
    question_two = Question.objects.create(poll=poll_two, text="Q2")
    Choice.objects.create(question=question_one, text="Yes", order=0)
    choice_two = Choice.objects.create(question=question_two, text="No", order=0)
...
def test_vote_requires_active_poll():
    ...
    choice = Choice.objects.create(question=question, text="Option", order=0)
    user = get_user_model().objects.create_user(username="voter", password="pw")
